-- =============================================================================
-- Increase truncation to 400 chars for premium locked posts
-- =============================================================================
...
BEGIN
  ...
  -- Truncate body if no access (400 chars for better preview)
  IF v_can_view THEN
    v_body := v_post.original_body;
    v_title := v_post.original_title;
  ELSE
    IF v_post.content_format = 'html' THEN
      v_body := LEFT(regexp_replace(v_post.original_body, '<[^>]+>', '', 'g'), 400);
    ELSE
      v_body := LEFT(v_post.original_body, 400);
    END IF;
    v_title := v_post.original_title;
  END IF;
