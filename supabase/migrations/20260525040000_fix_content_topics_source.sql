-- Fix: content_topics insert in create_native_attention missing 'source' column (NOT NULL violation)
-- Also adds 'confidence' column since it may also be required.
...
begin
  ...
  -- Link topics to the attention cluster (FIX: include source and confidence)
  if array_length(topic_slugs, 1) is not null then
    ...
    loop
      ...
      insert into public.content_topics (topic_id, target_id, target_type, source, confidence)
      values (topic_id_val, target_cluster_id, 'attention', 'user', 1.0)
      on conflict do nothing;
    end loop;
  end if;
